...
$actions = ['view', 'create', 'edit', 'delete', 'manage', 'export', 'approve', 'reconcile', 'reverse', 'retry'];

echo "Adding transactions module permissions...\n\n";
...
foreach ($actions as $action) {
    ...
    if (!$exists) {
        ...
        DB::table('permissions')->insert([
            ...
            'description' => 'Transactions - ' . ucfirst($action) . ' permission',
            'module' => $module,
            'action' => $action,
            'is_system' => false,
            'guard_name' => 'web',
...
echo "\n";
echo "Assigning transactions permissions to IT Manager role...\n\n";
...
// Get all transactions permissions
$permissionIds = DB::table('permissions')
    ->where('name', 'like', $module . '.%')
    ->pluck('id', 'name');

echo "Found " . count($permissionIds) . " transactions permissions.\n\n";
...
echo "\n";
echo "✅ Transactions permissions setup complete!\n";
echo "The IT Manager role now has full access to Transactions Management.\n";
echo "\n";

<div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex justify-between items-center mb-4">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Trade and Other Receivables</h2>
                <p class="text-sm text-gray-600 mt-1">Manage customer invoices, payments, and credit control</p>
            </div>
            <button wire:click="openCreateModal" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mt-6">
            <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-4 text-white">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-blue-100 text-sm">Total Receivables</p>
                        <p class="text-2xl font-bold mt-1">{{ number_format($totalReceivables, 2) }}</p>
                    </div>
                    <i class="fas fa-file-invoice text-2xl text-blue-200"></i>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-red-500 to-red-600 rounded-lg p-4 text-white">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-red-100 text-sm">Overdue Amount</p>
                        <p class="text-2xl font-bold mt-1">{{ number_format($totalOverdue, 2) }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-lg p-4 text-white">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-green-100 text-sm">Collected</p>
            ...
            <div class="bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg p-4 text-white">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-orange-100 text-sm">Bad Debt</p>
                        <p class="text-2xl font-bold mt-1">{{ number_format($totalBadDebt, 2) }}</p>
                    </div>
                    <i class="fas fa-times-circle text-2xl text-orange-200"></i>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg p-4 text-white">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-purple-100 text-sm">Avg Collection</p>
                        <p class="text-2xl font-bold mt-1">{{ $averageCollectionPeriod }} days</p>
                    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
        <div class="border-b border-gray-200">
            <nav class="flex -mb-px">
                ...
                <button wire:click="$set('activeTab', 'receivables')" 
                    class="px-6 py-3 text-sm font-medium {{ $activeTab == 'receivables' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700' }}">
                    <i class="fas fa-list mr-2"></i> Receivables List
                </button>
                <button wire:click="$set('activeTab', 'aging')" 
                    class="px-6 py-3 text-sm font-medium {{ $activeTab == 'aging' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700' }}">
                    <i class="fas fa-calendar-alt mr-2"></i> Aging Analysis
                </button>
                <button wire:click="$set('activeTab', 'collections')" 
                    class="px-6 py-3 text-sm font-medium {{ $activeTab == 'collections' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700' }}">
        ...
        <div class="p-6">
            @if($activeTab == 'overview')
                {{-- Aging Summary Chart --}}
                <div class="mb-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Aging Summary</h3>
                    <div class="grid grid-cols-1 lg:grid-cols-6 gap-4">
                        @foreach($agingBuckets as $bucket)
                            <div class="bg-gray-50 rounded-lg p-4">
                                <p class="text-sm text-gray-600">{{ $bucket['label'] }}</p>
                                <p class="text-xl font-bold text-gray-900 mt-1">{{ number_format($bucket['amount'], 2) }}</p>
                                <p class="text-xs text-gray-500 mt-1">{{ $bucket['count'] }} invoices</p>
                                <div class="mt-2">
                                    <div class="w-full bg-gray-200 rounded-full h-2">
                                        <div class="bg-blue-600 h-2 rounded-full" style="width: {{ $bucket['percentage'] }}%"></div>
                                    </div>
            ...
            @if($activeTab == 'receivables')
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
                    ...
                    <div>
                        <select wire:model="statusFilter" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                    ...
                    <div>
                        <select wire:model="ageFilter" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                    ...
                    <div>
                        <input type="date" wire:model="dateFrom" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                    </div>
                    <div>
                        <input type="date" wire:model="dateTo" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                    </div>
                </div>
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        @if($receivable->days_overdue > 0)
                                            <span class="text-red-600 font-medium">{{ abs($receivable->days_overdue) }} days overdue</span>
                                        @else
                                            {{ abs($receivable->days_overdue) }} days
                                        @endif
                                    </td>
                                        ...
                                        <div class="flex space-x-2">
                                            ...
                                            @if($receivable->status != 'paid' && $receivable->status != 'written_off' && $receivable->days_overdue > 120)
                                                <button wire:click="$emit('confirmWriteOff', {{ $receivable->id }})" 
                                                    class="text-red-600 hover:text-red-900" title="Write Off">
                        ...
                        @if($bucket['count'] > 0)
                            <div class="bg-gray-50 rounded-lg p-4">
                                <div class="flex justify-between items-center mb-3">
                                    <h4 class="text-lg font-semibold text-gray-900">{{ $bucket['label'] }}</h4>
                                    <div class="text-right">
                                        <p class="text-xl font-bold text-gray-900">{{ number_format($bucket['amount'], 2) }}</p>
                                        <p class="text-sm text-gray-500">{{ $bucket['count'] }} invoices ({{ $bucket['percentage'] }}%)</p>
                                    </div>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-3">
                                    <div class="bg-gradient-to-r from-blue-500 to-blue-600 h-3 rounded-full" style="width: {{ $bucket['percentage'] }}%"></div>
                                </div>
                            </div>
                        @endif
                    @endforeach
            ...
            @if($activeTab == 'collections')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-gray-50 rounded-lg p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">Collection Targets</h3>
                        <div class="space-y-3">
                            <div>
                    ...
                    <div class="bg-gray-50 rounded-lg p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">Top Debtors</h3>
                        <div class="space-y-3">
                            @php
                            ...
                            @foreach($topDebtors as $debtor)
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-600">{{ $debtor->customer_name }}</span>
                                    <span class="text-sm font-medium text-gray-900">{{ number_format($debtor->total_due, 2) }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                            ...
                            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select wire:model="customer_id" wire:change="updatedCustomerId" 
                                    ...
                                    <div>
                                        ...
                                        <input type="text" wire:model="invoice_number" {{ $editMode ? 'readonly' : '' }}
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm {{ $editMode ? 'bg-gray-100' : '' }}">
                                        @error('invoice_number') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                    </div>

                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Invoice Date</label>
                                        <input type="date" wire:model="invoice_date" wire:change="updatedInvoiceDate"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" wire:model="payment_terms" wire:change="updatedPaymentTerms"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" step="0.01" wire:model="amount" wire:change="updatedAmount"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" step="0.01" wire:model="vat_amount" wire:change="updatedVatAmount"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                    ...
                                    {{-- Account Selection --}}
                                    <div class="col-span-2 mt-4">
                                        <h4 class="text-sm font-medium text-gray-700 mb-2">Account Selection</h4>
                                    </div>

                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Receivable Account</label>
                                    ...
                                    <div>
                                        ...
                                        <select wire:model="income_account_id" 
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                            <option value="">Select Account</option>
                                    ...
                                    <div class="col-span-2 bg-gray-50 rounded-lg border border-gray-200 p-4">
                                        ...
                                        <p class="text-sm text-gray-600 mb-4">Select where to create the receivable account and the other account for double-entry posting</p>
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                <label for="parent_account_number" class="block text-sm font-medium text-gray-700 mb-2">
                                                    Parent Account (Create Receivable Under) *
                                                </label>
                                                ...
                                                <p class="text-xs text-gray-500 mt-1">New receivable account will be created under this parent</p>
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500 mt-1">Account to be credited (Revenue/Sales account)</p>
                            ...
                            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="file" wire:model="payment_receipt" 
                                            class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">

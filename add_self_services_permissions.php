...
$actions = ['view', 'create', 'edit', 'delete', 'manage', 'export', 'leave'];

echo "Adding self-services module permissions...\n\n";
...
foreach ($actions as $action) {
    ...
    if (!$exists) {
        ...
        DB::table('permissions')->insert([
            ...
            'description' => 'Self Services - ' . ucfirst($action) . ' permission',
            'module' => $module,
            'action' => $action,
            'is_system' => false,
            'guard_name' => 'web',
...
echo "\n";
echo "Assigning self-services permissions to IT Manager role...\n\n";
...
// Get all self-services permissions
$permissionIds = DB::table('permissions')
    ->where('name', 'like', $module . '.%')
    ->pluck('id', 'name');

echo "Found " . count($permissionIds) . " self-services permissions.\n\n";
...
echo "\n";
echo "✅ Self Services permissions setup complete!\n";
echo "The IT Manager role now has full access to Employee Self-Services.\n";
echo "\n";

...
class ComposePane extends Component
{
    use WithFileUploads;
    ...
    // Smart suggestions (to prevent deprecated property warnings)
    ...
    {
        // Initialize with empty arrays to prevent corruption
        $this->signatures = [];
        $this->selectedSignature = '';
        
        // Temporarily disable signature loading to debug corruption issue
        // try {
        //     $this->loadSignatures();
        //     $this->loadDefaultSignature();
        // } catch (Exception $e) {
        //     Log::error('[COMPOSE_PANE] Failed to initialize signatures', ['error' => $e->getMessage()]);
        //     // Continue with empty signatures rather than failing
        // }
    }
    ...
    public function loadDraft($draftId)
    {
        try {
            ...
            Log::error('[COMPOSE_PANE] Failed to load draft', ['error' => $e->getMessage()]);
    ...
    public function loadSignatures()
    {
        try {
            ...
            $signatures = $signatureService->getUserSignatures(Auth::id());
            
            // Convert to array and ensure proper structure
            if ($signatures) {
                $this->signatures = $signatures->map(function($sig) {
                    return [
                        'id' => $sig->id ?? null,
                        'name' => $sig->name ?? '',
                        'content' => $sig->content ?? '',
            ...
            Log::error('[COMPOSE_PANE] Failed to load signatures', ['error' => $e->getMessage()]);
            $this->signatures = [];
        }
    }
    ...
    public function loadDefaultSignature()
    {
        try {
            ...
            if ($signature && isset($signature->content)) {
                $this->selectedSignature = $signature->content;
            }
        } catch (Exception $e) {
            Log::error('[COMPOSE_PANE] Failed to load default signature', ['error' => $e->getMessage()]);
            $this->selectedSignature = '';
        }
    }
    ...
    {
        Log::info('[COMPOSE_PANE] Send email button clicked', [
            'user_id' => Auth::id(),
            'to' => $this->to,
        ...
        try {
            ...
            Log::error('[COMPOSE_PANE] Email send failed', [
                'error' => $e->getMessage(),
    ...
    public function saveDraft()
    {
        try {
            ...
            Log::error('[COMPOSE_PANE] Draft save failed', ['error' => $e->getMessage()]);
    ...
    public function discardEmail()
    {
        ...
        if ($this->draftId) {
            try {
                ...
                Log::error('[COMPOSE_PANE] Failed to delete draft', ['error' => $e->getMessage()]);
            }
        }
    ...
    {
        return view('livewire.email.compose-pane');
    }
}